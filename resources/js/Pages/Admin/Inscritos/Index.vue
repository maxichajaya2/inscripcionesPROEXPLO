...
// ==========================================
const exportToExcel = () => {
    // 1. Mapear y "aplanar" los datos para las columnas de Excel
    // Nota: Usamos filteredInscritos.value para que si el usuario busca algo,
    // solo se exporte lo que está viendo (o usa props.inscritos para exportar siempre todo)
    const dataToExport = filteredInscritos.value.map(inscrito => {

        // Formatear los cursos/viajes (como es un arreglo, los unimos con comas)
        const cursosText = inscrito.categoria_cursos_viajes && inscrito.categoria_cursos_viajes.length > 0
            ? inscrito.categoria_cursos_viajes.map(c => c.nombre_es).join(' | ')
            : 'Ninguno';

        return {
            'ID Inscripción': inscrito.id,
            'Fecha Registro': inscrito.fecha_registro,
            'Nombres Completos': inscrito.nombres,
            'Documento': inscrito.documento,
            'Correo Electrónico': inscrito.email,
            'Cargo': inscrito.cargo,
            'Origen': inscrito.origen,
            'Categoría Principal': inscrito.categoria_inscripcion?.nombre_es || 'No especificada',
            'Cursos / Viajes': cursosText,

            // Sección Facturación
            'Razón Social (Factura)': inscrito.facturacion?.razon_social || '-',
            'RUC (Factura)': inscrito.facturacion?.ruc || '-',
            'Dirección Facturación': inscrito.facturacion?.direccion || '-',
            'Correo Facturación': inscrito.facturacion?.correo_facturador || '-',
            'Monto Total ($)': inscrito.facturacion?.monto_total || 0,

            // Sección Pagos y Tarjeta
            'Estado Pago': inscrito.estado_pago,
            'Marca Tarjeta': inscrito.marca_tarjeta || 'Otra/Desconocida',
            'Número Tarjeta': inscrito.numero_tarjeta || '-',

            // Sección Cupones
            'Cupón Aplicado': inscrito.cupon ? inscrito.cupon.codigo : 'Ninguno',
            'Empresa del Cupón': inscrito.cupon ? inscrito.cupon.razon_social : '-',

            // Accesos
            'Código QR': inscrito.qr || 'No asignado',
            'Cupón de Viaje': inscrito.cupon_viaje || 'No asignado'
        };
    });

    // 2. Crear la hoja de cálculo a partir del JSON mapeado
    const worksheet = XLSX.utils.json_to_sheet(dataToExport);

    // 3. Crear el libro de trabajo y añadir la hoja
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Inscritos ProExplo");

    // 4. Forzar la descarga del archivo
    XLSX.writeFile(workbook, "Reporte_Inscritos.xlsx");
};
            ...
            <div
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                    ...
                    <button @click="exportToExcel"
                        class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl hover:bg-emerald-100 hover:border-emerald-300 transition-all text-sm font-bold shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
