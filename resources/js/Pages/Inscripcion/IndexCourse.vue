...
import { router, usePage } from '@inertiajs/vue3'; // Agregamos usePage
        ...
        <div class="px-6 py-12 mx-auto max-w-6xl min-h-[80vh] flex flex-col justify-center font-sans">
                ...
                <div class="p-3 md:p-6 flex items-center gap-3">
                    ...
                    <div class="text-left">
                        <div class="flex items-center gap-2 mb-0.5">
                            <span class="tag-proexplo text-[10px] md:text-xs px-2 py-0.5">{{ words.lbl_presale_rates }}</span>
                        </div>
                    ...
                    <div class="p-6 bg-slate-50/80 backdrop-blur-sm rounded-[2.5rem] border border-slate-200 shadow-xl overflow-hidden relative">
                        ...
                        <div class="relative z-10 flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <p v-if="cursosDetalle[0].categoria_description" class="text-orange-600 text-[10px] font-bold uppercase tracking-[0.2em]">
                        ...
                        <div class="grid grid-cols-1 gap-3">
                            <div v-if="cursosDetalle[0]" class="flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
                                <div class="w-3 h-3 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)] flex-shrink-0"></div>
                                <div class="flex-1">
                                    <p class="text-slate-900 font-extrabold text-lg leading-tight tracking-tight uppercase">
            ...
            <div class="flex flex-col lg:flex-row gap-8 items-start">
                <div class="w-full lg:w-4/12 space-y-4 animate-fade-in-right">
                    <p class="text-white/50 text-[10px] font-bold uppercase tracking-widest ml-2 mb-2">{{ words.lbl_filter_by_profile }}</p>
                        ...
                        <div v-for="cat in categoriasVisibles" :key="cat.id" @click="irAlFormulario(cat.id)"
                            ...
                            <div class="relative z-10 flex-1 pr-6">
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-green-100 text-green-700">
                                        PROEXPLO 2026
                                    </span>
                                </div>
                                <h4 class="text-lg md:text-xl font-bold text-slate-800 leading-tight group-hover:text-orange-700 transition-colors uppercase">
                                    {{ cat.nombre_en }}
                                </h4>
                            ...
                            <div class="relative z-10 text-right flex flex-col items-end border-l border-slate-100 pl-6 group-hover:border-orange-200 transition-colors">
                                <div class="mt-3 px-6 py-2 rounded-full bg-orange-600 text-white text-[10px] font-bold uppercase group-hover:bg-orange-700 transition-all shadow-sm">
                                    {{ words.btn_enroll }}
                                </div>
                            </div>
                        </div>

                        <div v-if="categoriasVisibles.length === 0" class="p-12 text-center border-2 border-dashed border-white/10 rounded-3xl animate-fade-in">
            ...
            <div class="w-full mt-12 animate-fade-in-up">
                <div class="flex flex-col md:flex-row justify-between items-center border-orange-200 bg-orange-50/30 rounded-3xl border p-6 md:p-8 gap-6">
                    ...
                    <div class="flex-1 text-center md:text-left">
                        <h6 class="text-xl font-black text-slate-800 uppercase mb-2">{{ words.lbl_other_participants }}</h6>
                        <p class="text-slate-600 leading-relaxed">
                            {{ words.msg_other_participants }}
                            <a href="mailto:inscripciones@iimp.org.pe" class="text-orange-600 font-bold hover:underline">
